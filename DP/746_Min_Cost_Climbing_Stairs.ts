...
//     DP = new Array(cost.length).fill(undefined);

//     return Math.min(cal(cost, 0), cal(cost, 1));
// }

// function cal(cost: number[], n: number): number {
//     if (n >= cost.length ) return 0;
//     if (n == cost.length - 2 || n == cost.length - 1) return cost[n];
...
function minCostClimbingStairs(cost: number[]): number {
    const n = cost.length;
    const dp = new Array(cost.length).fill(undefined);
